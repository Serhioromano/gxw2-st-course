...
- **Контакт** - Это 1-битная ячейка. Это может быть или ячейка которую можно только читать как `X100` так и **реле** типа `M50`, `Y10` когда из него происходит чтение. Получается что читать можно из любой 1-битной ячейки (**реле**), значит любая в контексте чтения является контактом.
- **Катушка** - Это 1-битная ячейка в которую можно писать в контексте записи. Например `M50`, `Y10`, `TC0`, и т.д. Когда мы в нее пишем называем катушкой.
