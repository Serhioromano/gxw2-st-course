...
## 5️⃣ Рабочая область: Ваше рабочее пространство

А теперь давайте перейдем к самому сердцу программы — **Рабочей области**. Это центральная и самая большая часть экрана, где происходит вся магия: мы пишем код, настраиваем переменные, отлаживаем программу. Представьте, что это ваш основной рабочий стол, на котором лежат документы.

### Система вкладок: Порядок в документах

Каждый открытый документ в проекте — будь то программа, список переменных или что-то еще — представлен в виде **вкладки** в верхней части этой области. Это очень похоже на то, как работают вкладки в вашем интернет-браузере.

* **Идентификация:** На каждой вкладке есть ее **имя** и небольшая **иконка**, которая помогает мгновенно понять, с каким типом документа вы работаете.
* **Переключение:** Просто кликая по разным вкладкам, вы меняете документ, который отображается в основном окне. Вот, сейчас у нас открыты два документа: "Редактор переменных" и "Редактор программы". Я кликаю по одной — вижу переменные, кликаю по другой — вижу код.
* **Закрытие:** Чтобы закрыть ненужный документ, достаточно нажать на маленький крестик на его вкладке. Если вы случайно что-то закрыли, не беда — всегда можно снова открыть документ, сделав **ДК** на нем в дереве проекта.

### Навигация при большом количестве вкладок

Иногда в работе бывает нужно открыть десять, а то и двадцать документов одновременно. В этом случае вкладки просто перестают помещаться на экране. Разработчики предусмотрели это.

* **Стрелки прокрутки:** Справа от вкладок появляются маленькие **кнопки со стрелками**. Нажимая на них, вы можете "прокручивать" ленту вкладок влево и вправо, чтобы добраться до нужной. (На экране демонстрируется уменьшение окна и прокрутка).
* **Выпадающий список:** В самом правом углу есть кнопка с **треугольником**. Она открывает выпадающий список, в котором перечислены **все** открытые на данный момент документы. Это самый быстрый способ переключиться на нужный документ, если их открыто очень много.

### Контекстное меню вкладок: Управление окнами

Есть еще одна очень удобная функция для наведения порядка. Если нажать **ПКМ** на любой вкладке, откроется контекстное меню, которое дает вам полный контроль над окнами:

* **Закрыть:** Закрывает текущую вкладку.
* **Закрыть все окна справа/слева:** Позволяет закрыть все документы, расположенные справа или слева от текущего.
* **Закрыть все, кроме этого:** Оставляет только текущий документ, закрывая все остальные. Этой командой я пользуюсь постоянно, чтобы сфокусироваться на одной задаче.
* **Закрыть все окна:** Закрывает абсолютно все открытые документы.

### Горячие клавиши для навигации: Работаем как профессионалы

Конечно же, перемещаться по вкладкам можно и с помощью клавиатуры, что гораздо быстрее, чем тянуться к мыши.

* **Переключение:**
    * ++ctrl+tab++ или ++ctrl+"F6"++: Переключиться на **следующую** вкладку (вправо).
    * ++ctrl+shift+tab++: Переключиться на **предыдущую** вкладку (влево).
* **Цикличность:** Важный момент: навигация "закольцована". Это значит, что если вы дошли до самой последней вкладки и нажмете ++ctrl+tab++ еще раз, вы перейдете на самую первую. И наоборот. (На экране демонстрируется с тремя открытыми документами).
* **Закрытие:** Чтобы закрыть текущий активный документ, используйте сочетание ++ctrl+"F4"++.

### Многооконный режим: Видеть все и сразу

По умолчанию каждый документ занимает всю рабочую область. Но часто в работе возникает необходимость видеть содержимое двух или даже трех окон одновременно. Например, смотреть на список переменных и одновременно писать код, который их использует.

Важно: в отличие от навигаторов, вкладки документов **нельзя** просто так перетащить и расположить рядом. Для этого существует специальное меню **Window** (Окно). Давайте рассмотрим его ключевые команды.

#### 1. Cascade (Каскад)

Эта команда превращает каждую вкладку в отдельное, свободно плавающее окно внутри рабочей области. Представьте, что вы разложили на столе стопку бумажных документов веером.

* **Управление:** Эти окна можно свободно **перемещать**, **изменять их размер** и даже **сворачивать**. Свернутое окно превращается в маленький заголовок в нижней части рабочей области, откуда его можно снова развернуть.
* **Активное окно:** Окно, с которым вы работаете в данный момент, всегда будет поверх остальных.
* **Навигация:** Что самое удобное, все горячие клавиши (вроде ++ctrl+tab++) продолжают работать и в этом режиме, позволяя быстро "поднять" нужное окно наверх, сделав его активным.

#### 2. Tile Vertically и Tile Horizontally (Расположить плиткой)

Эти две команды также превращают документы в отдельные окна, но, в отличие от каскада, они **автоматически** располагают их на экране, чтобы они занимали все пространство без перекрытий. Это похоже на то, как располагаются панели в комиксе.

* **Tile Vertically** (++alt+"W"+"V"++): Располагает окна друг рядом с другом по вертикали. Я очень часто использую этот режим, когда мне нужно работать с двумя документами. Я просто закрываю все лишнее, оставляю два нужных окна и нажимаю эту комбинацию. И вуаля — идеальное рабочее пространство.
* **Tile Horizontally** (++alt+"W"+"H"++): Располагает окна друг над другом по горизонтали.

Как программа решает, как их расположить? Она пытается сделать это максимально равномерно.

* **2 окна:** Идеально делят экран пополам (вертикально или горизонтально).
* **3 окна:** Также располагаются вполне ожидаемо в ряд или столбик.
* **4 и более окон:** Программа начинает автоматически выстраивать сетку, пытаясь выделить каждому окну примерно равную площадь. (На экране демонстрируется с 4 и 5 окнами).

Этот механизм автоматического расположения позволяет вам очень быстро организовать рабочее пространство под конкретную задачу, не тратя время на ручное перетаскивание и изменение размеров окон.
